    ...
    <>
      {threads.length > 0 && (
        <>
          ...
          {threads.map(
            ([mapThreadId, { creationDate, messages }], index) =>
              mapThreadId && (
                <Card
                  key={index}
                  className={`flex items-center justify-between p-4 hover:bg-muted/50 focus:bg-muted/50 ${
                    threadId === mapThreadId ? 'bg-zinc-300' : ''
                  }`}
                  onClick={() => {
    ...
    <>
      ...
      <div
        className={`fixed inset-0 z-50 transition-all duration-300 ${
          isMenuOpen ? 'bg-black bg-opacity-50' : 'pointer-events-none'
        }`}
      >
        <div
          className={`fixed left-0 h-full w-full sm:w-3/4 bg-white transform transition-transform duration-300 ${
